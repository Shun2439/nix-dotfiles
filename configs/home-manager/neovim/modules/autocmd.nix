  ...
  programs.nixvim = {
    autoCmd = [
      ...
      # Set indentation for specific file types
      {
        event = "FileType";
        pattern = [ "nix" ];
        callback = { __raw = "function() vim.opt_local.tabstop = 2 vim.opt_local.shiftwidth = 2 end"; };
      }
      {
        event = "FileType";
        pattern = [ "python" ];
        callback = { __raw = "function() vim.opt_local.tabstop = 4 vim.opt_local.shiftwidth = 4 end"; };
      }
      {
        event = "FileType";
